import { Source, ASTNode, Kind, visit, BREAK, getVisitFn, TypeInfo, GraphQLSchema } from 'graphql';
...
export function positionFromPositionInContainingDocument(source: Source, position: Position) {
  if (!source.locationOffset) return position;
  return Position.create(position.line - (source.locationOffset.line - 1), position.character);
}

export function positionInContainingDocument(source: Source, position: Position): Position {
  if (!source.locationOffset) return position;
  return Position.create(source.locationOffset.line - 1 + position.line, position.character);
}
...
export function positionFromSourceLocation(source: Source, location: SourceLocation) {
  return Position.create((source.locationOffset ? source.locationOffset.line - 1: 0) + location.line - 1, location.column - 1);
}
